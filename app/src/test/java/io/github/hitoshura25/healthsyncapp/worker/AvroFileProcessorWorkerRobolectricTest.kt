...
class AvroFileProcessorWorkerRobolectricTest {
    ...
    fun testWorker_processesAllFileTypes_movesToCompleted_andSavesData() = runBlocking {
        ...
        createAvroFileInDir(stagingDir, "HeartRateRecord_test1.avro", avroHeartRateRecords)
        ...
        val avroSleepSessionRecords = listOf(
        ...
        createAvroFileInDir(stagingDir, "SleepSessionRecord_test1.avro", avroSleepSessionRecords)

        // Blood Glucose
        val avroBloodGlucoseRecords = listOf(
            AvroBloodGlucoseRecord(
                hcUid = "bg-uid-1", 
                timeEpochMillis = currentTime - 15000L, 
                levelValue = 100.0, 
                levelUnit = AvroBloodGlucoseLevelUnit.MILLIGRAMS_PER_DECILITER, 
                specimenSource = AvroSpecimenSource.CAPILLARY_BLOOD, 
                mealType = AvroMealType.BREAKFAST, 
                relationToMeal = AvroRelationToMeal.BEFORE_MEAL, 
        ...
        createAvroFileInDir(stagingDir, "BloodGlucoseRecord_test1.avro", avroBloodGlucoseRecords)
        ...
        assertThat(savedSteps?.count).isEqualTo(500L)
        assertThat(savedSteps?.hcUid).isEqualTo("steps-uid-1")
        ...
        assertThat(savedHrSamples[0].beatsPerMinute).isEqualTo(75L)
        assertThat(savedHrSamples[0].hcRecordUid).isEqualTo("hr-uid-1") // Assumes HeartRateSampleEntity.hcRecordUid exists

        val savedSleepSession = sleepSessionDao.getRecordByHcUid("sleep-uid-1")
        assertThat(savedSleepSession).isNotNull()
        assertThat(savedSleepSession?.title).isEqualTo("Night Sleep")
        assertThat(savedSleepSession?.hcUid).isEqualTo("sleep-uid-1")

        val savedSleepStages = sleepStageDao.getStagesBySessionHcUid("sleep-uid-1")
        assertThat(savedSleepStages).isNotEmpty()
        assertThat(savedSleepStages.size).isEqualTo(2)
        assertThat(savedSleepStages.any { it.stage == AvroSleepStageType.LIGHT.name && it.sessionHcUid == "sleep-uid-1" }).isTrue() // Assumes SleepStageEntity.sessionHcUid exists
        assertThat(savedSleepStages.any { it.stage == AvroSleepStageType.DEEP.name && it.sessionHcUid == "sleep-uid-1" }).isTrue()

        val savedBg = bloodGlucoseDao.getRecordByHcUid("bg-uid-1")
        assertThat(savedBg).isNotNull()
        assertThat(savedBg?.levelMgdL).isEqualTo(100.0)
        assertThat(savedBg?.hcUid).isEqualTo("bg-uid-1")
    }
